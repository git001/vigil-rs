...
pub struct Cli {
    ...
    #[arg(
        long,
        env = "SOURCE_PROXY_CACERT",
        value_name = "PATH",
        help_heading = "Source Connection"
    )]
    pub source_proxy_cacert: Option<PathBuf>,

    /// Comma-separated list of hosts/CIDRs that bypass the source proxy.
    ///
    /// Each entry is matched against the request host (port stripped).
    /// Supports exact hostnames (`internal.corp`), domain suffixes
    /// (`.corp` matches `api.corp`), IPv4 CIDRs (`192.168.0.0/16`),
    /// and IPv6 CIDRs (`fd00::/8`).
    /// Applies to --source-url and --kubernetes modes.
    #[arg(
        long,
        env = "SOURCE_NO_PROXY",
    ...
    pub source_no_proxy: Option<String>,
    ...
    #[arg(long, value_name = "NAME|all|n1,n2,...", help_heading = "Kubernetes")]
    ...
    /// Disable the K8s stream= query parameter and use a single combined
    /// stdout+stderr stream regardless of the API server version.
    ///
    ...
    #[arg(
        long,
        env = "DEST_CONNECT_TIMEOUT",
        default_value = "10000",
        value_name = "MS",
        help_heading = "TCP Sink Connection"
    )]
    ...
    #[arg(
        long,
        env = "DEST_RECONNECT_DELAY",
        default_value = "500",
        value_name = "MS",
        help_heading = "TCP Sink Reconnect"
    )]
